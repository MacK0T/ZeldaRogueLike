...
public class Room
{
	...
	public int GetValidDirection(int num_tries)
	{
		...
		{
			if (x >= dungeon.worldX - 1) return GetValidDirection(num_tries+1);
			if (GetRight() != null) return GetValidDirection(num_tries+1);
		}
		else if (direction == 2) // Top
		{
			if (y >= dungeon.worldY - 1) return GetValidDirection(num_tries+1);
	...
	{
		int tileX = x + 1;
		if (tileX >= dungeon.worldX) return null;
	...
	{
		int tileY = y + 1;
		if (tileY >= dungeon.worldY) return null;
	...
	{
		int n = 0;
		if (GetTop() == null && y < dungeon.worldY - 1) n++;
		if (GetBottom() == null && y > 0) n++;
		if (GetLeft() == null && x > 0) n++;
		if (GetRight() == null && x < dungeon.worldX - 1) n++;
		return n;
	}
}